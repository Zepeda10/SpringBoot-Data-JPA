...
public class ClienteController {
	 ...
	 * @Autowired
	 * 
	 * @Qualifier("ClienteDaoJPA") //Para especificar a cuál Beans se está
	 * refiriendo el autowired private IClienteDao clienteDao;
	 */
	...
	public String listar(@RequestParam(name = "page", defaultValue = "0") int page, Model model, Authentication authentication, HttpServletRequest request) {
		...
		Page<Cliente> clientesPaginados = clienteService.findAll(pageRequest);
		PageRender<Cliente> pageRender = new PageRender<>("/listar", clientesPaginados);
	...
	@PreAuthorize("hasRole('ROLE_USER')") // Hace la misma función que el @Secured("ROLE_USER")
	@GetMapping("/ver/{id}")
